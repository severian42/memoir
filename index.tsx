...
class LivingMemoirApp {
  ...
  private async saveState(): Promise<void> {
    ...
    try {
      ...
      // Backup storage: IndexedDB for larger data
      await this.saveToIndexedDB(state);

      // Create automatic backup every 10 saves
      const saveCount = parseInt(localStorage.getItem('memoir_save_count') || '0') + 1;
      localStorage.setItem('memoir_save_count', saveCount.toString());
      ...
      try {
        ...
        console.error('Critical: Unable to save data:', fallbackError);
        this.updateSaveStatus('error');
        alert('Warning: Unable to save your data. Please export your memoir as backup.');
      }
    }
  }
  ...
  private updateSaveStatus(status: 'saving' | 'saved' | 'error'): void {
    ...
    switch (status) {
      case 'saving':
        ...
        this.saveStatus.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
        this.saveStatus.title = 'Saving...';
        break;
      case 'saved':
        this.saveStatus.innerHTML = '<i class="fas fa-check-circle"></i>';
        this.saveStatus.title = 'All changes saved';
        break;
      case 'error':
        this.saveStatus.classList.add('error');
        this.saveStatus.innerHTML = '<i class="fas fa-exclamation-triangle"></i>';
        this.saveStatus.title = 'Save failed - please export as backup';
        break;
    }
  }
  ...
  private async loadState(): Promise<void> {
    try {
      ...
      if (indexedDBState) {
        this.applyState(indexedDBState);
        return;
      }
    } catch (error) {
  ...
  private async loadFromIndexedDB(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const db = request.result;
        const transaction = db.transaction(['memoir'], 'readonly');
        const store = transaction.objectStore('memoir');
  ...
  private async createBackup(state: any): Promise<void> {
    try {
      const backupKey = `memoir_backup_${Date.now()}`;
      localStorage.setItem(backupKey, JSON.stringify(state));

      // Keep only last 3 backups
      const backupKeys = Object.keys(localStorage)
        .filter(key => key.startsWith('memoir_backup_'))
        .sort()
        .reverse();
    ...
    try {
      ...
      for (const key of backupKeys) {
        try {
          ...
          console.warn('Backup recovery failed for:', key, error);
        }
      }
    } catch (error) {
      console.error('Complete recovery failure:', error);
    }
  }
  ...
  private async proceedWithRecording(question: Question, card: HTMLElement): Promise<void> {
    ...
    try {
      ...
      this.mediaRecorder.onstop = async () => {
        ...
        const textArea = card.querySelector('.text-input') as HTMLTextAreaElement;
        if (textArea.value.trim()) {
          statusEl.textContent = 'Transcription complete. Add photo or save.';
        } else {
          statusEl.textContent = 'Recording complete. Please type your response or try recording again.';
      ...
      this.startLiveTranscription(card);

      this.mediaRecorder.start();
      this.isRecording = true;
      statusEl.textContent = 'Recording with live transcription... Click to stop.';
  ...
  private startLiveTranscription(card: HTMLElement): void {
    if (!this.speechRecognition || this.isTranscribing) return;
    ...
    this.speechRecognition.onerror = (event) => {
      ...
      if (event.error === 'not-allowed') {
        statusEl.textContent = 'Microphone permission denied. Recording audio only.';
      }
    };
    ...
    try {
      ...
      statusEl.textContent = 'Recording... (Live transcription unavailable)';
    }
  }
  ...
  private async getTranscription(base64Audio: string, mimeType: string): Promise<string | null> {
    // This method is no longer used since we do live transcription
    // But keeping it for compatibility
    return "Live transcription was used during recording.";
  }
  ...
  private exportData(): void {
    try {
      ...
      alert('Your memoir data has been exported successfully!');
    } catch (error) {
  ...
  private async handleDataImport(event: Event): Promise<void> {
    ...
    try {
      ...
      if (!this.validateImportedData(importedState)) {
        alert('Invalid data format. Please select a valid memoir backup file.');
        return;
      }
      ...
      const confirmImport = confirm(
        ...
        'Make sure you have exported your current data as backup. Continue?'
      );
